...
public enum BackendError: LocalizedError {
    ...
    /// Privacy-modus Local-only (v1): er mogen geen fotobytes de Mac verlaten.
    /// Gegooid door de cloud-beeldendpoints vóór enige upload, als de
    /// `cloudAllowed`-gate van de client `false` geeft.
    ...
    public var errorDescription: String? {
        ...
        case .cloudAIDisabled:
            return "This feature needs Cloud AI. Change your privacy setting to use it."
        }
    }
}
...
public final class BackendClient {
    ...
    /// Optionele privacy-gate (v1 Local-only-modus): geeft `false` als er
    /// geen fotobytes naar de cloud mogen. `nil` = altijd toegestaan (Aaavatar
    /// 2 regelt dit in de feature-laag). Fail-closed vóór elke upload.
    public var cloudAllowed: (() -> Bool)?

    private func requireCloudAI() throws {
        if let cloudAllowed, !cloudAllowed() { throw BackendError.cloudAIDisabled }
    }
    ...
    // MARK: POST /v1/auth/send-recovery-email (v1 Pro-herstel, E01.7)
    /// Stuurt een herstel-link naar een e-mailadres (anoniem toegestaan).
    /// Anti-enumeratie-contract: de server antwoordt identiek voor bekende en
    /// onbekende adressen; de enige client-zichtbare onderscheidingen zijn
    /// 400 (ongeldig adres) en 429 (rate-limit). De UI-copy is altijd "If we
    /// have an account, we sent a link". Gebruikt door v1's RecoverProSheet;
    /// hier geland bij de merge van main in v2-main (v1's eigen BackendClient
    /// verdween in E01.5).
    public enum RecoveryEmailError: LocalizedError {
        case invalidEmail
        case rateLimited
        case unknown
        public var errorDescription: String? {
    ...
    public func sendRecoveryEmail(_ email: String) async throws {
        struct Body: Encodable { let email: String }
        struct Sent: Decodable { let sent: Bool }
        let body = try JSONEncoder().encode(Body(email: email))
        do {
            let _: Sent = try await requestAllowingAnonymous(
            ...
            // 5xx, transport, decode → één generieke fout; de oorspronkelijke
            // error mag door het anti-enumeratie-contract toch niet getoond.
