...
public struct VisionCutoutEngine: CutoutEngine {
    ...
    public func cutout(_ image: CGImage) async throws -> CGImage {
        ...
        if let personBuffer = personSeg.results?.first?.pixelBuffer {
            ...
            // Omgekeerde gate (PersonGate): de subject-lift telt alleen mee in
            // een zone rond de persoon-matte — een decoratieve schijf achter de
            // persoon blijft niet staan. Zonder bruikbare persoon-matte
            // ongewijzigd.
            let foregroundBase = PersonGate.apply(matte: fgMask, person: person, extent: extent)
