...
**Result (herziening, Thierry 2026-07-12):** de UI toont nu maar één betaalde optie — de gratis on-device boost dekt het lichte geval al, dus de google-middenweg verviel uit het menu. `BoostMode` terug naar local/online; online = Topaz High Fidelity V2 à 3 credits (chip + StylizeQualityCoordinator-copy volgen `CreditMeter.chipLabel(for: .upscaleHigh)`). De backend houdt béide tiers: `quality:"regular"` blijft het contract voor oude builds zonder veld (google, 1 cr — kostendekkend) en als dev-arm. build-v2.sh opnieuw volledig groen.
**Result (deploy):** ✅ prod-deploy 2026-07-12 op go van Thierry (Vercel CLI vanaf worktree-root, rootDirectory=backend; deployment `avatars-ehxlopj8q…` Ready). Prod-smoke: `/v1/upscale`+`/v1/cutout`+`/v1/stylize` zonder auth → 401, `/v1/effects` + `/v1/banner-presets` → 200. Vanaf nu draait élke online-boost van bestaande builds op regular (google, 1 cr) — de verlieslatende Topaz-runs zijn gestopt; High komt beschikbaar zodra de app-kant meegemerged is.
