...
#   ./scripts/release-v2.sh --check          # structuur + versie-assert, geen build
#
...
s = pathlib.Path("project.yml").read_text()
mv = [m.group(0) for m in re.finditer(r'MARKETING_VERSION: "[^"]*"', s)]
bv = [m.group(0) for m in re.finditer(r'CURRENT_PROJECT_VERSION: "[^"]*"', s)]
if len(mv) != 2 or len(bv) != 2:
    print(f"❌ verwachtte precies 2× MARKETING_VERSION en 2× CURRENT_PROJECT_VERSION (root=v1, target=v2); vond {len(mv)} / {len(bv)}", file=sys.stderr)
    sys.exit(1)
if "https://api.aaavatar.nl/appcast-v2.xml" not in s:
    print("❌ Avatar2 SUFeedURL moet appcast-v2.xml zijn", file=sys.stderr)
    sys.exit(1)
if "https://api.aaavatar.nl/appcast.xml" not in s:
    print("❌ v1 SUFeedURL moet appcast.xml blijven", file=sys.stderr)
    sys.exit(1)
print("project.yml: 2 versielijnen + gescheiden Sparkle-feeds OK")
PY
}
...
for name in ("appcast-v2.xml", "backend/api/_appcast-v2.xml"):
    root = ET.parse(name).getroot()
    print(f"{name} ok — {sum(1 for _ in root.iter('item'))} item(s)")
a = pathlib.Path("appcast-v2.xml").read_text()
b = pathlib.Path("backend/api/_appcast-v2.xml").read_text()
if a != b:
    print("❌ appcast-v2.xml en backend/api/_appcast-v2.xml lopen uit elkaar", file=sys.stderr)
    sys.exit(1)
print("appcast-v2 lockstep OK")
PY
  echo "✅ release-v2 --check groen"
  exit 0
fi
...
echo "  4. v1 /appcast.xml en releases/latest/download/Aaavatar.dmg ongemoeid laten"
